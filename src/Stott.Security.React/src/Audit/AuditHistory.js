        ...
        return auditHistory && auditHistory.map(auditEntry => {
            ...
            return(
                <Card id={id} className='my-3' key={id}>
                    <Card.Header><strong>{recordType}</strong> was <strong>{operationType}</strong> by <strong>{actionedBy}</strong> on <strong>{format(new Date(actioned), "Do MMM yyyy [at] HH:mm:ss")}</strong></Card.Header>
                    <Card.Body>
