...
function EditPermissionPolicy(props) {
    ...
    const { getPermissionPolicyDirectives } = useContext(PermissionPolicyContext);
    ...
    const handleRemoveSource = (idToRemove) => {
        let newSpecificSources = specificSources.filter((e) => e.id !== idToRemove);
        setSpecificSources(newSpecificSources);
        setHasSourcesError(false);
    };

    const handleUpdateSource = (idToUpdate, sourceUrl) => {
        let newSpecificSources = specificSources.map(x => x.id === idToUpdate ? { ...x, url: sourceUrl } : x);
        setSpecificSources(newSpecificSources);
        setHasSourcesError(false);
    };
    ...
    const renderSources = () => {
        return specificSources && specificSources.map((directiveSource) => {
            return (
                <FormWildcardSourceUrl key={directiveSource.id} sourceId={directiveSource.id} sourceUrl={directiveSource.url} handleDeleteSource={handleRemoveSource} handleUpdateSourceUrl={handleUpdateSource} allowWildcard={true} />
            );
        });
    };
    ...
    const handleSaveDirective = () => {
        ...
        axios.post(import.meta.env.VITE_APP_PERMISSION_POLICY_SOURCE_SAVE, payload)
            .then(() => {
