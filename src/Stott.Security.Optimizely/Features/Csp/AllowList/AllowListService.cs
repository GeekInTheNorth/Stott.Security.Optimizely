...
internal sealed class AllowListService : IAllowListService
{
    ...
    public async Task AddFromAllowListToCspAsync(string? violationSource, string? violationDirective, Guid? siteId, string? hostName)
    {
        var settings = await _cspSettingsService.GetAsync(siteId, hostName);
        ...
        try
        {
            ...
            {
                await _cspPermissionService.AppendDirectiveAsync(allowListMatch.SourceUrl, violationDirective, "Allow List Automation", siteId, hostName);
            }
        }
    ...
    public async Task<bool> IsOnAllowListAsync(string? violationSource, string? violationDirective, Guid? siteId, string? hostName)
    {
        var settings = await _cspSettingsService.GetAsync(siteId, hostName);
        if (!settings.IsAllowListEnabled
            || string.IsNullOrWhiteSpace(violationSource)
