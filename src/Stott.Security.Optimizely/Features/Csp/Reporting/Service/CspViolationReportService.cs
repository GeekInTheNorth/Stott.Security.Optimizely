...
internal sealed class CspViolationReportService : ICspViolationReportService
{
    ...
    {
        return await _repository.GetReportAsync(source, directive, threshold, siteId, hostName);
    }

    public async Task SaveAsync(ICspReport violationReport, Guid? siteId, string? hostName)
    {
        ...
        {
            await _repository.SaveAsync(blockedUri, validDirective, siteId, hostName);
        }
    }
